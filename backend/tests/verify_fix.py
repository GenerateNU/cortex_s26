...
async def test_preprocess_flow():
    ...
    mock_repo.get_extraction_with_file_info.return_value = {
        "raw_files": {
            "file_name": file_name,
            "file_link": "path/to/test_document.pdf"
        }
    }
    ...
    try:
        with patch('app.services.preprocess_service.generate_embedding', new_callable=AsyncMock) as mock_embedding:
    ...
    if call_args:
        ...
        if passed_file_name == file_name:
            print(f"SUCCESS: file_name '{passed_file_name}' was passed to update_extraction_result")
        else:
            print(f"FAILURE: Expected file_name '{file_name}', got '{passed_file_name}'")
    else:
