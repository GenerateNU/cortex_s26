...
class PatternRecognitionService:
    ...
    async def analyze_relationships(
        ...
        files_resp = (
            await self.supabase.table("extracted_files")
        ...
        for f in files:
            ...
            # Check if linked
            linked_resp = (
                await self.supabase.table("file_relationships")
    ...
    async def detect_and_link(
        self, file_id: UUID, summary: str, manual_context: str = None
    ) -> None:
        """
        ...
        response = (
            await self.supabase.table("relationships")
            .select("relationship_name, relationship_description")
            .execute()
        )
        ...
        existing_list_str = "\n".join(
            [
                f"- {r['relationship_name']}: {r['relationship_description']}"
        ...
        for match in matches:
            ...
            existing_match = next(
                (
                    r
                    for r in existing_relationships
                    if r["relationship_name"].lower() == rel_name.lower()
                ),
                None,
            )
            ...
            if existing_match:
                ...
                r_group = (
                    await self.supabase.table("relationships")
                    .select("relationship_id")
                    .eq("relationship_name", existing_match["relationship_name"])
                    .single()
                    .execute()
                )
                if r_group.data:
            ...
            if rel_id:
                ...
                try:
                    await (
                        self.supabase.table("file_relationships")
                        .insert(
                            {
                                "file_id": str(file_id),
    ...
    async def get_graph_data(self) -> dict[str, list[Any]]:
        ...
        for link in links:
            ...
            file_name = (
                link["raw_files"]["file_name"] if link["raw_files"] else "Unknown File"
            )

            if file_id not in seen_files:
                nodes.append({"id": file_id, "label": file_name, "type": "file"})
